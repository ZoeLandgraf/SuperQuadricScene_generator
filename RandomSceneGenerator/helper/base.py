...
from helper import extra, get_collision_file, geometry
...
class SceneGenerationBase:
    ...
    def _is_contained(self, unique_id):
        threshold = 0.5
        ratio = extra.pybullet.aabb_contained_ratio(
    ...
    def _is_colliding(self, unique_id):
        ...
        # check collision
        is_colliding = False
        for other_unique_id in extra.pybullet.unique_ids:
    ...
    def _spawn_object(self, class_id):
        ...
        unique_id = extra.pybullet.add_model(
            visual_file=cad_file,
            collision_file=get_collision_file.get_collision_file(cad_file),
    ...
    def unique_id_to_pose(self, unique_id):
        ...
        T_cad2world = geometry.compose_transform(
    ...
    def _render_pyrender(self, T_camera2world, fovy, height, width):
        ...
        node_camera = scene.add(
            ...
            pose=extra.trimesh.to_opengl_transform(T_camera2world),
        )
        for _ in range(4):
    ...
    def _render_pybullet(self, T_camera2world, fovy, height, width):
        rgb, depth, ins = extra.pybullet.render_camera(
    ...
    def debug_render(self, T_camera2world):
        ...
        scene = extra.pybullet.get_trimesh_scene()
        ...
        scene.camera_transform = extra.trimesh.to_opengl_transform(
            T_camera2world
        )
        ...
        rgb = extra.trimesh.save_image(scene)[:, :, :3]
        ins_viz = imgviz.label2rgb(ins + 1, rgb)
        cls_viz = imgviz.label2rgb(
            cls, rgb, label_names=class_names, font_size=20
        )
    ...
    def random_camera_trajectory(
        ...
        targets = geometry.trajectory.sort(targets)
        targets = geometry.trajectory.interpolate(
